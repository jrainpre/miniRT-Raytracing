...
void	fill_dummy_scene(t_scene *scene)
{
	...
	if (scene->cam == NULL)
		return ;
	scene->cam->orig = (t_pt){0, 0, 10};
	scene->cam->orientation = (t_vec){0, 0, -1};
	scene->cam->fov = 70;
	...
	scene->cam->vertical = (t_vec){0, scene->cam->sensor_height, 0};
	scene->cam->horizontal = (t_vec){scene->cam->sensor_width, 0, 0};
	scene->cam->lower_left_corner = vec_sub(scene->cam->orig, vec_add(vec_add(vec_div(scene->cam->horizontal, 2), vec_div(scene->cam->vertical, 2)), (t_vec){0, 0, scene->cam->focal_length}));

	//spheres
	sphere = malloc_or_print_error(sizeof(t_sphere));
	if (sphere == NULL)
		return ;
	sphere->orig = (t_pt){0, 0, -100};
	sphere->radius = 40.0;
